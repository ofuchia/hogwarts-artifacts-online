...
public class UserService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        return this.userRepository.findByUserName(username)//find user from db
                .map(hogwartsUser -> new MyUserPrincipal(hogwartsUser)) //if found, wrap the returned instance in a myUserPrincipal instance
                .orElseThrow(() -> new UsernameNotFoundException("username " + " is not found.")); //else throw an exception
    }
}